...
from antlr.parser.generated.CompiscriptParser import CompiscriptParser  # noqa
from antlr.sema.ast_builder import ASTBuilder  # noqa
from antlr.sema.checker import Checker  # noqa
...
st.markdown(
    """
    <style>
    :root { --bg:#0f172a; --panel:#111827; --muted:#1f2937; --acc:#22c55e; --acc2:#06b6d4; --warn:#f59e0b; --err:#ef4444; --txt:#e5e7eb; --sub:#9ca3af; }
    ...
    .title { padding:8px 14px; border-radius:10px; background: linear-gradient(90deg, var(--acc), var(--acc2)); color:#0b1220; font-weight:800; display:inline-block; letter-spacing:.3px; }
    .panel { background:var(--panel); border:1px solid var(--muted); border-radius:14px; padding:14px; box-shadow:0 1px 12px rgba(0,0,0,.18); }
    ...
    .metric-box { background:linear-gradient(180deg, rgba(31,41,55,.6), rgba(17,24,39,.6)); border:1px solid var(--muted); border-radius:12px; padding:12px; text-align:center; }
    ...
    .stTextArea[data-baseweb="textarea"]:has(textarea:disabled) textarea { opacity:.9; }
    .stButton > button { background: linear-gradient(180deg, #1f2937, #111827); border:1px solid #243244; color:var(--txt); border-radius:10px; padding:.45rem .9rem; }
    .stButton > button:hover { border-color:#2f415a; }
    .statusbar { margin-top:6px; display:flex; gap:16px; align-items:center; justify-content:flex-end; background:var(--panel); border:1px solid var(--muted); border-radius:10px; padding:6px 10px; }
...
def _parse_sem_line(msg: str):
    if (len(msg) >= 4) and (msg[0] == "["):
        ...
        while i < len(msg) and msg[i].isdigit():
            lnum = lnum * 10 + (ord(msg[i]) - 48)
            i += 1
        if i < len(msg) and msg[i] == ":":
            i += 1
            cnum = 0
            while i < len(msg) and msg[i].isdigit():
                cnum = cnum * 10 + (ord(msg[i]) - 48)
                i += 1
            if i < len(msg) and msg[i] == "]":
                i += 1
                if i < len(msg) and msg[i] == " ":
...
def _norm_diag_from_dict(e: Dict[str, Any], default_kind: str) -> Dict[str, Any]:
    ...
    for k in ("kind", "category", "type", "phase"):
        v = e.get(k)
        kind = v if isinstance(v, str) and v else kind
    for k in ("message", "msg", "text", "detail", "description", "reason"):
        v = e.get(k)
        msg = v if isinstance(v, str) else msg
...
def _join_params(plist: List[Dict[str, Any]]) -> str:
    ...
    while i < len(plist):
        p = plist[i]
        nm = p.get("name", "")
        tp = p.get("type", "unknown") or "unknown"
        parts.append(nm + ":" + tp)
        i += 1
...
def build_ir_and_asm(src_code: str) -> tuple[str, str]:
    ...
    listener = _SyntaxErrorListener()
    parser.removeErrorListeners()
    parser.addErrorListener(listener)

    tree = parser.program()
    if len(listener.errors) > 0:
        raise ValueError("Errores de sintaxis:\n" + "\n".join(listener.errors))
    ...
    with c2:
        ...
        if len(tokens) == 0:
            ...
            if st.button("Consultar (token seleccionado)"):
                if 0 <= idx < len(tokens):
                    ln = int(tokens[idx].get("line", 1))
                    cl = int(tokens[idx].get("col", 0))
                    try:
...
if show_ast:
    with tabs[t]:
        ...
        dot = result.get("astDot") if isinstance(result, dict) else None
        if dot:
            try:
