...
def T_INT():       return Type('int')
def T_FLOAT():     return Type('float')
def T_BOOL():      return Type('bool')
def T_STRING():    return Type('string')
def T_VOID():      return Type('void')
def T_NULL():      return Type('null')
def T_UNKNOWN():   return Type('unknown')
def T_ARRAY(elem): return Type('array', elem)
def T_CLASS(name): return Type('class', name)
def T_FUNC(params, ret): return Type('func', (params, ret))


# Predicados de tipo
def is_numeric(t): return t is not None and (t.kind == 'int' or t.kind == 'float')
def is_int(t):     return t is not None and t.kind == 'int'
def is_float(t):   return t is not None and t.kind == 'float'
def is_bool(t):    return t is not None and t.kind == 'bool'
def is_string(t):  return t is not None and t.kind == 'string'
def is_array(t):   return t is not None and t.kind == 'array'
def is_class(t):   return t is not None and t.kind == 'class'
def is_func(t):    return t is not None and t.kind == 'func'
def is_void(t):    return t is not None and t.kind == 'void'
def is_null(t):    return t is not None and t.kind == 'null'
def is_unknown(t): return t is not None and t.kind == 'unknown'
...
def assignable(src, dst):
    ...
    if can_widen(src, dst): return True
    # null a tipos de referencia (array/class/string)
    if is_null(src) and is_reference_like(dst): return True
...
def unify_numeric(a, b):
    if not (is_numeric(a) and is_numeric(b)): return None
    if is_float(a) or is_float(b): return T_FLOAT()
    return T_INT()
...
def binary_result(op, lt, rt):
    # Lógicos
    if op == '&&' or op == '||':
        if is_bool(lt) and is_bool(rt): return T_BOOL()
        return None

    # Aritméticos
    if op == '+' or op == '-' or op == '*' or op == '/':
        # Soporte string + string -> string (opcional; el resto, no permitido)
        if op == '+' and is_string(lt) and is_string(rt): return T_STRING()
    ...
    if op == '==' or op == '!=' or op == '<' or op == '<=' or op == '>' or op == '>=':
        if compare_compatible(lt, rt, op): return T_BOOL()
        return None
...
def ternary_unify(a, b):
    ...
    if is_null(a) and is_reference_like(b): return b
    if is_null(b) and is_reference_like(a): return a
    return None


# Arrays / Índices / Literales
def index_result(arr_t, idx_t):
    if not is_array(arr_t): return None
    if not is_int(idx_t): return None
    return arr_t.info
...
def call_compatible(fun_t, arg_types):
    ...
    if not is_func(fun_t): return (False, -1)
    params, ret = fun_t.info
    if len(params) != len(arg_types): return (False, -1)
    i = 0
    while i < len(params):
